...
*MICHAEL*: Need to show source for this document
...
```{r}
ucd_heart %>%
  filter(is.na(Deaths) & is.na(GEOID))
...
*MICHAEL*: Why is EPSG 2163 used? Need to explain EPSG 
...
```{r}
ucd_heart %>%
  filter(GEOID==46113|GEOID==46102)
```
...
oglala_correction <- ucd_heart %>%
  filter(GEOID=="46113") %>%
  mutate(GEOID="46102", County="Oglala Lakota")

contiguous_us <- contiguous_us %>%
  left_join(oglala_correction, by="GEOID") %>%
...
**MICHAEL** This entire section needs to be checked. Changes were made to ensure that there was no trunctating effect on the analysis. 
...
**MICHAEL** Everything having to do with the subregion needs to be looked over again and possibly removed. 
...
**Michael**: Must show the dimensions of the distance matrix. Distance matrix is a bit unclear at this moment to the audience of this case study.
...
```{r, eval=FALSE}
us_cvd_sp_data <- us_cvd_sp_data %>% filter(Region =="Region of Interest")
```

**MICHAEL** This needs to be updated to reflect "Great Circle" distance with the `st_distance` function as shown earlier in this case study. 

**MICHAEL** Constant error shows below. This should be fixed and then explained when done earlier in this case study. The code fixing the warning/constant at the moment is not explained well and almost appears arbitrary. Must read some more about this. 

**Michael**: Code below is suitable for distances but not deaths. Needs to be adapted for deaths. 
...
**Michael** The math above for the death imputation needs to be checked as the crude rate is in a unit that could inadvertently multiply the number of deaths by a multiple of 10. 

**Michael** There is an NA value that may cause a problem.
...
```{r}
moran.mc(us_cvd_sp_data$residuals6nb, listw =FFct_nb_w_test,nsim = 9999, zero.policy = TRUE)
```

**MICHAEL**: This needs to be revisited. 
