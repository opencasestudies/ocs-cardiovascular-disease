...
The learning objectives include data wrangling and spatially lagged models.
...
We will be obtaining demographic and geographic information from the US Census Bureaum, our first source of data. Our analysis will use demographic information from the American Community Survey, which functions similarly to the US Census, but is conducted yearly. Two map files for this analysis will be considered. You can obtain these map files [here](https://www.census.gov/geographies/mapping-files/time-series/geo/carto-boundary-file.2017.html) and here. 

*We need to use the below link* 
...
Let's import the United States county boundaries data. Note that we are importing two different files. These two files have different resolutions. We will determine which of the two files to use. 
...
If we plot the boundaries for the City of Baltimore (a county), we can see that there are is substantially more detail in one of files. This could be useful when generating maps that are smaller in scale or when the resolution could impact an analysis.

```{r}
BALT_shape <- map_sf %>% filter(NAME == "Baltimore", GEOID==24510)
BALT_shape_high_res <- map_sf_high_res %>% filter(NAME == "Baltimore", GEOID==24510)

ggplot() +
  geom_sf(data=BALT_shape, color=NA, fill="blue", alpha=0.25) +
  geom_sf(data=BALT_shape_high_res, color=NA, fill="red", alpha = 0.25) +
  labs(title = "Impact of resolution of spatial data on shape produced") +
  theme_minimal()
```

If we zoom out, we see that the resolutions do make make much of a difference.

```{r}
MD_shape <- map_sf %>% filter(STATEFP==24)
MD_shape_high_res <- map_sf_high_res %>% filter(STATEFP==24)

ggplot() +
  geom_sf(data=MD_shape, color=NA, fill="blue", alpha=0.25) +
  geom_sf(data=MD_shape_high_res, color=NA, fill="red", alpha = 0.25) +
  labs(title = "Impact of resolution of spatial data on shape produced") +
  theme_minimal()
```

Since we are not conducting an analysis that uses information directly from the boundaries we use, we will use lower resolution shapefile. This shapefile requires less computational resources and produces almost identical maps at the state level and beyond. 
...
med_age <- read_csv("ACS_17_1YR_B01002/ACS_17_1YR_B01002_with_ann.csv")
...
```{r}
ucd_heart <- ucd_heart %>% rename("GEOID"="County Code")
```

```{r}
map_sf <- map_sf %>% left_join(ucd_heart, by="GEOID")
...
tm_shape(subset) +
	tm_polygons(c("Deaths", "Population", "Crude Rate"),
				style=c("pretty","pretty","pretty"), breaks=list(NULL, NULL, NULL),
				palette=list("Oranges", "Purples", "Reds"),
				border.col = "white",
				title=c("Deaths", "Population", "Crude Rate")) + tm_facets(as.layers = TRUE)
