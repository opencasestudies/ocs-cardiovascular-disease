...
It looks like there may be residual spatial autocorrelation. 
...
plot(moran_cor_m5,ylim=c(-0.05, 0.4),main="Residuals from model 5")
plot(moran_cor_m5nb,ylim=c(-0.05, 0.4),main="Residuals from model 5, nb")
```
...
```{r, error=TRUE}
# no 2013 urbanization category, population_lag introduced
...
plot(moran_cor_m6,ylim=c(-0.05, 0.4),main="Residuals from model 6")
plot(moran_cor_m6nb,ylim=c(-0.05, 0.4),main="Residuals from model 6, nb")
```
...
```{r}
moran.mc(matrix_region$residuals6nb, listw =FFct_nb_w_test,nsim = 999, zero.policy = TRUE)
```
