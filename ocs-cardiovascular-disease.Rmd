...
Let's import the United States county boundaries data. Note that we are importing two different files. These two files have different resolutions. We must determine which of the two files to use. 
...
Since we are not conducting an analysis that uses information directly from the boundaries we use, we will use lower resolution shapefile. This shapefile requires less computational resources and produces almost identical maps at the state level and beyond, meaning even choropleths we produce during our exploratory analysis will be very accurate representations of the data.
...
Wrangle total population to help get accurate crude rate.
